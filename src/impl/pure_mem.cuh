...
#pragma unroll
  ...
  __global__ void __launch_bounds__(128, 14)
    ...
    // Direct GPU memory access helpers (no PageData wrapper)
    const int page_size = 4096;
    auto get_node_ptr = [&] (int nodeid) -> uint8_t* {
      int block = nodeid / nodes_per_page;
      int offset = (nodeid % nodes_per_page) * node_len;
      return data + (int64_t)block * page_size + offset;
    };
    ...
    for (int qid = blockIdx.x; qid < qcnt; qid += gridDim.x) {
      ...
      while(u != -1) {
        uint8_t* node_ptr = get_node_ptr(u);
        node_data = (T*)node_ptr;
        int* meta = (int*)(node_ptr + data_len);
        deg = meta[0];
        edge = meta + 1;
        ...
        if (tid >= deg) {
          ...
          int nodeid_v = tmp_id[sz + tid] = edge[tid];
          float* dist_vec = pq_data->pq_dists + bid * PQSearchData::num_pivots * pq_data->num_chunks;
          float dist = 0;
          uint8_t* cdata = pq_data->compressed_data + (long) nodeid_v * pq_data->num_chunks;
#pragma unroll 32
          for (int j = 0; j < pq_data->num_chunks; j++) {
            dist += dist_vec[j * PQSearchData::num_pivots + cdata[j]];
          }
          tmp_dist[sz + tid] = dist;
        }
...
#pragma unroll
          ...
          float dist = square_sum_32(src_vec, node_data, num_dims);
          retset_push_32(distances + qid * topk, nns + qid * topk, found_cnt[qid],
                         topk, dist, u);
        }
        __syncthreads();
