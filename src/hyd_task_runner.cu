...
namespace gustann {
  ...
  static __global__ void init_search(float* qdata, PQSearchData* pq_data, int stream_offset, int dim) {
    pq_data->init_query(qdata + blockIdx.x * dim, stream_offset);
  }
  
  static __global__ void copy_page(uint8_t* dest, uint8_t* src, int32_t* request, const int data_len, const int data_cnt) {
    int tid = threadIdx.x;
    assert(data_len % 4 == 0);
  ...
  void TaskRunner::init_query(const float* qdata, int _qcnt, int* _nns, float* _dis, int *_found_cnt, int *_start_pt) {
